package com.darksoldier1404.dlec.functions;
...
public class DLECFunction {
    private static final LegendaryEnderChest plugin = LegendaryEnderChest.getInstance();
    ...
    public static void openStorage(Player p, Player target) {
        ...
        ItemStack block = NBT.setStringTag(new ItemStack(Material.BARRIER), "dlec-block", "true");
    ...
    public static void saveCurrentContents(DInventory inv) {
        ...
        for (int i = 0; i < 45; i++) {
            Inventory inv1 = (Inventory) inv;
            content[i] = inv1.getContents()[i];
        }
    ...
    public static void saveInventory(DInventory inv) {
        ...
        for (ItemStack[] item : inv.getPageItems().values()) {
            for (int i = 0; i < item.length; i++) {
                if (!NBT.hasTagKey(item[i], "dlec-block")) {
    ...
    public static void getCoupon(Player p, String sint) {
        ...
        p.getInventory().addItem(NBT.setIntTag(coupon, "dlec-coupon", i));
    ...
    public static void useCoupon(Player p, ItemStack item) {
        int i = NBT.getIntegerTag(item, "dlec-coupon");
        YamlConfiguration data = plugin.udata.get(p.getUniqueId());
        int owned = data.getInt("Owned");
        int maxPages = plugin.maxPages + 1;
        if(owned + i + plugin.defaultSlot > (maxPages * 45)) {
            p.sendMessage(ColorUtils.applyColor("&a엔더창고를 확장할 수 있는 최대 슬롯을 넘었습니다."));
            return;
        }
        data.set("Owned", owned + i);
        p.sendMessage(ColorUtils.applyColor("&a엔더창고 확장 쿠폰을 사용했습니다."));
