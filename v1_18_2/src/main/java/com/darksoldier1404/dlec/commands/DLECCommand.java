package com.darksoldier1404.dlec.commands;
...
public class DLECCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args[0].equals("오픈")) {
            ...
            if (args.length == 2) {
                if (p.hasPermission("dlec.admin")) {
                    DLECFunction.openStorage(p, Bukkit.getPlayer(args[1]));
                    return false;
                }else{
                    p.sendMessage(plugin.prefix + "권한이 없습니다.");
                    return false;
        ...
        if (args[0].equals("쿠폰")) {
            if (p.hasPermission("dlec.admin")) {
                ...
                if (args.length == 2) {
                    DLECFunction.getCoupon(p, args[1]);
                    return false;
                }
            }
        }
